  ...
  methods: {
    isUserLogged() {
      if (!this.identity || !this.principal || this.principal === Principal.anonymous()) {
        throw new Error("PLZ log in as bombelek!")
      }
      return {
    ...
    async login() {
      ...
      await authClient.login({
        identityProvider: "http://c2lt4-zmaaa-aaaaa-qaaiq-cai.localhost:4943/",
        onSuccess: async () => {
...
<template>
  <main>
    ...
    <div v-if="principal && !userData">
      <input v-model="newUsername" placeholder="bombelek name"/> <button @click="registerUsername">register</button>
    </div>
    <div v-if="principal && userData">
      {{ userData.nickname }}
      <div v-if="allUsers">
        <select v-model="targetPrincipal" @change="getAllUsers">
      ...
      <div>
        <textarea v-model="newChat" placeholder="bombelek message"></textarea><button @click="dodajChatMSG">Wyślij bombelekMSG</button>
      </div>
    </div>
  </main>
</template>